...
- **Anything true of one Worktree lives in `.data`, not config**: pin, custom label, last-used Launch profile, queued prompts, history. Observation never deletes it; only a console-initiated Remove does, so a checkout that vanishes from git (unmounted, deleted by hand) leaves its records until an explicit prune.
