...
function ClientSettingsMenu({ settings }: { settings: ClientSettings }) {
  ...
  const settingsRows = <div className="client-settings-overview"><div className="client-settings-setting client-settings-identity" role="group" aria-label="Client"><header><small>CLIENT</small><span className="client-settings-setting-actions"><button type="button" aria-label="Rename Client" onClick={() => beginRename('client')}>Rename</button></span></header><strong>{settings.deviceName}</strong><span>This browser</span></div><div className="client-settings-setting client-settings-identity" role="group" aria-label="Server"><header><small>SERVER</small><span className="client-settings-setting-actions"><button type="button" aria-label="Rename Server" onClick={() => beginRename('server')}>Rename</button></span></header><strong>{settings.serverName}</strong><span>{serverHostLabel(settings.serverUrl)}</span></div>{terminalFontSetting}{themeSetting}{agentsSetting}</div>;
...
function GitStatus({ branch, summary, prSummary, expanded = false, onToggle, onOpenFile, onReview, reviewOpen = false, reviewUnavailable }: { branch?: string; summary?: GitStatusSummary; prSummary?: GitComparisonSummary; expanded?: boolean; onToggle?: () => void; onOpenFile: (path: string) => void; onReview?: (scope: ReviewScope) => void; reviewOpen?: boolean; reviewUnavailable?: string }) {
  ...
  useLayoutEffect(() => {
    ...
    const syncPanelPosition = () => {
      ...
      // open upward, left-aligned to the status chip (now in the bottom action row), clamped to the viewport
...
function AgentCard({ agent, active, tabBar, cleanupControl, reviewCapability, review, onReview, onDeleted, onSelectTarget, onPromptFocus, onOperationFeedback, pinned, onTogglePin, onRenameWorktree, worktreeLabel }: { agent: Agent; active: boolean; tabBar: ReactNode; cleanupControl?: ReactNode; reviewCapability?: ReviewTourCapability; review?: ReviewButtonState; onReview: (launch: ReviewLaunch) => void; onDeleted: () => Promise<void>; onSelectTarget: (target: DashboardTarget) => void; onPromptFocus: () => void; onOperationFeedback: (feedback: Omit<OperationFeedback, 'id'>) => void; pinned?: boolean; onTogglePin?: () => void; onRenameWorktree?: () => void; worktreeLabel?: string }) {
  ...
  // the git status chip and prompt-history control live in Log (with its notes/git-preview
  // hooks) but render into these slots in the bottom composer row, via portals (there is no
  // upper toolbar anymore); Prompt owns the slot elements, Log fills them
...
function WorktreeCard({ worktree, tabBar, cleanupControl, onLaunched, onTurnedOff, onOperationFeedback, onRename, onRemove, removeDisabledReason }: { worktree: Worktree; tabBar: ReactNode; cleanupControl?: ReactNode; onLaunched: (agentId: string, worktree: Worktree, operationKey: string) => void; onTurnedOff: () => Promise<void>; onOperationFeedback: (feedback: Omit<OperationFeedback, 'id'>) => void; onRename: () => void; onRemove?: () => void; removeDisabledReason?: string }) {
