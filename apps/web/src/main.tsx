...
function ResizableLogSplit({ worktreeId, output, note, browser, terminals, onPhoneTerminal }: { worktreeId?: string; output: ReactNode; note?: ReactNode; browser?: ReactNode; terminals?: TerminalColumn[]; onPhoneTerminal?: (paneId: string | undefined) => void }) {
  ...
  const mobileSwitches = hasSplit ? <div className="mobile-split-switches" role="toolbar" aria-label="Split view">{columns.filter(column => column.key !== visibleMobilePanel).map(column => {
    ...
    const label = kind === 'agent' ? 'Show agent output' : kind === 'note' ? 'Show note' : kind === 'browser' ? 'Show project browser' : `Show terminal ${termLabel || termIndex + 1}`;
...
function GitStatus({ id, worktreeId, branch, summary, prSummary, pullRequest, onFixup, expanded = false, onToggle, onOpenFile, onReview, reviewOpen = false, reviewUnavailable, pushAction = defaultPushAction, pushPending = false, onPush, onSelectTarget, onOperationFeedback }: { id?: string; worktreeId?: string; branch?: string; summary?: GitStatusSummary; prSummary?: GitComparisonSummary; pullRequest?: PullRequestSummary; onFixup?: () => Promise<boolean>; expanded?: boolean; onToggle?: () => void; onOpenFile: (path: string) => void; onReview?: (scope: ReviewScope) => void; reviewOpen?: boolean; reviewUnavailable?: string; pushAction?: PromptAction; pushPending?: boolean; onPush?: () => Promise<boolean>; onSelectTarget?: (target: DashboardTarget) => void; onOperationFeedback?: (feedback: Omit<OperationFeedback, 'id'>) => void }) {
...
function WorktreeCard({ worktree, tabBar, cleanupControl, onLaunched, onTurnedOff, onOperationFeedback, onNavigateWorktree, onRename, onRemove, removeDisabledReason, schedulePrefill }: { worktree: Worktree; tabBar: ReactNode; cleanupControl?: ReactNode; onLaunched: (agentId: string, worktree: Worktree, operationKey: string) => void; onTurnedOff: () => Promise<void>; onOperationFeedback: (feedback: Omit<OperationFeedback, 'id'>) => void; onNavigateWorktree: (worktreeId: string) => void; onRename: () => void; onRemove?: () => void; removeDisabledReason?: string; schedulePrefill?: SchedulePrefill }) {
  ...
  const gitStatus = <GitStatus branch={worktree.branch} summary={worktree.gitStatus} prSummary={worktree.gitPrStatus} pullRequest={worktree.pullRequest} expanded={gitExpanded} onToggle={() => { /* toggle file inspection */ setGitExpanded(value => !value); }} onOpenFile={openGitFile} reviewUnavailable="Launch agent to review" />;
