...
function AgentCard({ agent, active, onDeleted }: { agent: Agent; active: boolean; onDeleted: () => Promise<void> }) {
  ...
  return <article className="agent-view"><Log id={agent.id} onOpenTerminal={() => setTerminal(true)} onQuestion={setQuestion} /><Prompt id={agent.id} canCancel={active} cancelling={cancelling} deleting={deleting} onCancel={() => void cancel()} onDelete={!active && agent.worktreeId === undefined ? () => void remove() : undefined} projectUrl={agent.projectUrl} pullRequestUrl={agent.pullRequestUrl} question={omxQuestion ?? question} worktreeId={agent.worktreeId} stack={agent.stack} />{terminal && <div className="dialog" role="dialog" aria-modal="true"><div><button onClick={() => setTerminal(false)}>Close</button><Terminal agent={agent} /></div></div>}</article>;
}
