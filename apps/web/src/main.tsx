...
const worktreeRemoveDisabledReason = (worktree: Pick<Worktree, 'locked' | 'main'>, project: Pick<Project, 'manageWorktrees' | 'manageWorktreesReason'> | undefined): string | undefined =>
  worktree.main ? 'The main worktree cannot be removed'
    ...
    : undefined;
// what a launcher row calls a Worktree. Rows nest under their Project's header, so the
// Project prefix the wire label carries ("Repo · feat") is dropped; Main carries no
// suffix, so it falls back to its branch
const launcherRowLabel = (worktree: Worktree, project: Project): string =>
  worktree.label.startsWith(`${project.label} · `) ? worktree.label.slice(project.label.length + 3) : worktree.branch ?? worktree.label;
...
// the glyphs on a launcher row's icon controls; the pin's head fills once the row is pinned
const launcherRowIcons = { pin: 'M9 4h6l-1 6 3 3v2H7v-2l3-3-1-6ZM12 15v5', trash: 'M4 7h16M10 7V4.5h4V7M6.5 7l.9 12.1a1 1 0 0 0 1 .9h7.2a1 1 0 0 0 1-.9L17.5 7M10 11v5M14 11v5' };
