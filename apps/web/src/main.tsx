...
// split does not jump.
function CodePane({ controller }: { controller: CodePanelController }) {
  return <Suspense fallback={<section className="code-pane" role="region" aria-label="Code changes"><p className="code-pane-status">Loading changes…</p></section>}>
    <CodePanel mode={controller.mode} state={controller.state} patch={controller.patch} loadFile={controller.loadFile} onClose={controller.close} onRetry={controller.refresh} />
  </Suspense>;
}
...
function Log({ id, agentWorking = false, worktreeId, branch, gitStatus, gitPrStatus, pullRequest, history, refreshHistory, onQuestion, onMetadata, cleanupControl, browserUrl, browserHomeUrl, browserProxied = true, browserNavigationRequest, onBrowserNavigate, onBrowserOpen, onBrowserClose, code, embedded = false, onReview, reviewOpen = false, reviewUnavailable, pushAction = defaultPushAction, processingLabel, processingDetail, statusSlot, historySlot, onSelectTarget, onNavigateWorktree, onOperationFeedback, schedulePrefill, terminals, onPhoneTerminal }: LogProps) {
  ...
  const codePane = embedded || code === undefined || !code.open ? null : <CodePane controller={code} />;
...
function WorktreeCard({ worktree, tabBar, cleanupControl, onLaunched, onTurnedOff, onOperationFeedback, onNavigateWorktree, onRename, onRemove, removeDisabledReason, schedulePrefill }: { worktree: Worktree; tabBar: ReactNode; cleanupControl?: ReactNode; onLaunched: (agentId: string, worktree: Worktree, operationKey: string) => void; onTurnedOff: () => Promise<void>; onOperationFeedback: (feedback: Omit<OperationFeedback, 'id'>) => void; onNavigateWorktree: (worktreeId: string) => void; onRename: () => void; onRemove?: () => void; removeDisabledReason?: string; schedulePrefill?: SchedulePrefill }) {
  ...
  const codePane = projectCode.open ? <CodePane controller={projectCode} /> : null;
