...
export function ProjectOpen({ url, projectProxied, stack, browserOpen = false, onBrowserToggle, onStackAction, onStackLog }: { url?: string; projectProxied?: boolean; stack?: ProjectStack; browserOpen?: boolean; onBrowserToggle?: () => void; onStackAction?: (action: StackAction) => Promise<unknown> | unknown; onStackLog?: () => Promise<StackOperationLog | undefined> }) {
  ...
  // label stack-only running state
  const stackStatus = stack?.running === true ? 'running' : stack?.running === false ? 'stopped' : 'unknown';
  const stackStatusClass = stackStatus === 'running' ? 'live' : stackStatus === 'stopped' ? 'disconnected' : 'inactive';
  useEffect(() => {
  ...
  return <><span className={`project-open-group${url === undefined ? ' stack-only' : ''}${hasBrowserControl ? ' has-browser-control' : ''}${hasStackActions ? ' has-stack-actions' : ''}`} ref={anchorRef} role="group" aria-label={url === undefined ? 'Stack controls' : 'Project controls'}>{url !== undefined && <a className={`project-open status-${status}${unavailable ? ' disabled' : ''}${inProgress ? ' busy' : ''}`} href={url} target="_blank" rel="noreferrer" aria-disabled={unavailable || inProgress || undefined} aria-busy={inProgress || undefined} title={title} onClick={openProject}>{inProgress ? <span className="spinner" aria-hidden="true" /> : <i aria-hidden="true" />}{label}</a>}{hasBrowserControl && url !== undefined && <button className="project-browser-toggle icon-button" type="button" disabled={browserUnavailable} aria-label="Open project in split view" aria-pressed={browserOpen} title={browserUnavailable ? 'Split view unavailable' : 'Split view'} onClick={onBrowserToggle}><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="4" width="18" height="16" rx="1" /><path d="M12 4v16" /></svg></button>}{hasStackActions && <button className="project-stack-toggle icon-button" type="button" disabled={inProgress} aria-label={url === undefined ? `Stack controls: ${stackStatus}` : 'Stack controls'} aria-expanded={menuOpen} title={url === undefined ? `Stack controls · ${stackStatus}` : 'Stack controls'} onClick={() => setMenuOpen(open => !open)}>{url === undefined ? <span className={`status ${stackStatusClass}`} aria-hidden="true"><i /><svg viewBox="0 0 24 24"><path d="m6 9 6 6 6-6" /></svg></span> : <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 9 6 6 6-6" /></svg>}</button>}</span>{menuOpen && <FlyoutPortal onDismiss={() => setMenuOpen(false)}><div className="stack-menu more-menu flyout-menu" ref={flyoutRef} style={style}>{actions.map(action => <button key={action} disabled={inProgress} onClick={() => void run(action)}>{operation === action ? <><span className="spinner" />{stackOperationLabel(action)}…</> : stackActionLabel(action)}</button>)}{hasStackLogs && <><hr className="more-menu-divider" /><button className="stack-log-menu-button" type="button" onClick={() => { setMenuOpen(false); openLogs(); }}><svg className="more-menu-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M5 6h14v12H5zM8 10l2 2-2 2M12 14h4" /></svg>Show output</button></>}</div></FlyoutPortal>}{logDialog}</>;
}
