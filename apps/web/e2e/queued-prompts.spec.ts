...
test('manages waiting prompts from the clock control connected to Queue', async ({ page }) => {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (url.pathname === '/api/dashboard') return route.fulfill({ json: { generation: 1, agents: [{ id: 'agent-1', sessionId: 'socket:$1', workspace: '/worktrees/cora', worktreeId: 'cora', title: '⠋ Working', attention: 'working' }], projects: [] } });
