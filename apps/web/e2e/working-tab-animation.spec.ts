...
test('animates working tabs from left to right with an orange status dot', async ({ page }) => {
  await page.route('**/api/**', async route => {
    ...
    if (url.pathname === '/api/dashboard') return route.fulfill({ json: { generation: 1, agents: [{ id: 'agent-1', sessionId: 'socket:$1', workspace: '/worktrees/cora', worktreeLabel: 'Cora', title: '⠋ Working', attention: 'working' }], worktrees: [] } });
...
test('animates an unread success tab with a green glow and status dot', async ({ page }) => {
  await page.route('**/api/**', async route => {
    ...
    const url = new URL(request.url());
    if (url.pathname === '/api/auth/session') return route.fulfill({ json: { csrfToken: 'csrf-token', active: true, deviceName: 'Test device' } });
    if (url.pathname === '/api/dashboard') return route.fulfill({ json: { generation: 1, agents: [{ id: 'agent-1', sessionId: 'socket:$1', workspace: '/worktrees/cora', worktreeLabel: 'Cora', title: 'Ready' }, { id: 'agent-2', sessionId: 'socket:$2', workspace: '/worktrees/delta', worktreeLabel: 'Delta', title: 'Ready', unread: true }], worktrees: [] } });
    if (url.pathname === '/api/push/public-key') return route.fulfill({ json: {} });
