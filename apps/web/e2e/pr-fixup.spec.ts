...
test('queues $fixup from a PR card with detected issues', async ({ page }) => {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (url.pathname === '/api/dashboard') return route.fulfill({ json: { generation: 1, agents: [{ id: 'agent-1', sessionId: 'socket:$1', home: '/worktrees/cora', title: 'Ready', pullRequest: { number: 42, title: 'Repair this PR', status: 'open', url: 'https://github.com/octo/repo/pull/42', issues: { failingChecks: true } } }], projects: [] } });
