...
test('bookmarks the current chat above notes and resumes a selected bookmark', async ({ page }) => {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (bookmarkMatch && request.method() === 'DELETE') {
      deletedBookmark = bookmarkMatch[1]!;
    ...
    if (switchMatch && request.method() === 'POST') {
      switchedBookmark = switchMatch[1]!;
  ...
  await expect(bookmarkButton).toHaveAccessibleName('Bookmarked chats (0)');
  ...
  const [menuBounds, nameBounds, rowPresentation] = await Promise.all([
    ...
    menu.locator('.bookmark-row').first().evaluate(element => {
      ...
      return { overflow: row.overflow, radius: row.borderRadius, choiceRadius: choice.borderRadius, choiceBorder: choice.borderWidth };
    })
  ]);
  ...
  await menu.getByRole('button', { name: 'Bookmark this chat' }).click();
  await expect.poll(() => bookmarkedCurrent).toBe(1);
  await expect(bookmarkButton).toHaveAccessibleName('Bookmarked chats (1)');

  await menu.locator('.bookmark-choice').filter({ hasText: 'Implement worktree chat bookmarks' }).click();
