...
test('opens a changed file in the Code panel from the git flyout while new output arrives', async ({ page }) => {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (url.pathname === '/api/dashboard') return route.fulfill({ json: { generation: 1, agents: [{ id: 'agent-1', worktreeId: 'cora', sessionId: 'socket:$1', workspace: '/worktrees/cora', branch: 'feature/file-preview', gitStatus: { files: 1, staged: 0, unstaged: 1, untracked: 0, conflicted: 0, changes: [{ code: ' M', path: filePath, additions: 12, deletions: 3 }] }, title: 'Ready' }], projects: [] } });
