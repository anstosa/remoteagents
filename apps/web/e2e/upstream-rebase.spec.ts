...
test('queues the tracked upstream rebase workflow from the active branch banner', async ({ page }) => {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (url.pathname === '/api/dashboard') return route.fulfill({ json: { generation: 1, agents: [{ id: 'agent-1', sessionId: 'socket:$1', home: '/worktrees/cora', worktreeId: 'cora', worktreeLabel: 'Cora', branch: 'feature/console', gitUpstream: { upstream: 'origin/feature/console', ahead: 1, behind: 2 }, title: 'Ready' }], projects: [] } });
