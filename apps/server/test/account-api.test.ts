...
describe('Codex account API', () => {
  ...
  it('queries safe limits and restarts only open idle worktrees after switching', async () => {
    ...
    const firstCora = stated({ id: 'agent-cora-1', paneId: '%1', sessionId: 'socket:$1', socketFingerprint: 'socket', home: cora.path, worktreeId: cora.id, title: 'Ready' });
    const secondCora = { ...firstCora, id: 'agent-cora-2', paneId: '%2', sessionId: 'socket:$2' };
    const workingOwen = stated({ id: 'agent-owen', paneId: '%3', sessionId: 'socket:$3', socketFingerprint: 'socket', home: owen.path, worktreeId: owen.id, title: '⠋ Working' });
    const scratch = stated({ id: 'agent-scratch', paneId: '%4', sessionId: 'socket:$4', socketFingerprint: 'socket', home: '/tmp', title: 'Ready' });
  ...
  it('preserves a prompt that starts while an account switch selects restart targets', async () => {
    ...
    const hash = await argon2.hash('synthetic-password', { type: argon2.argon2id });
    const cora = testWorktree({ id: 'cora', projectId: 'cora', label: 'Cora', path: '/worktrees/cora', pinned: false });
    const idleCora = stated({ id: 'agent-cora', paneId: '%1', sessionId: 'socket:$1', socketFingerprint: 'socket', home: cora.path, worktreeId: cora.id, title: 'Ready' });
    const socket = { fingerprint: 'socket', path: '/tmp/tmux', device: 1, inode: 2 };
