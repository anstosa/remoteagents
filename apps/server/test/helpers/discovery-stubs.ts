...
/**
 * A ProcessInspector: `codex` controls `hasCodexDescendant`; provide `sessions`
 * to enumerate rollout sessions per pid.
 */
export const processInspector = (options: { codex?: boolean; sessions?: (pid: number) => CodexSessionRef[] } = {}): ProcessInspector => ({
  hasCodexDescendant: async () => options.codex ?? true,
