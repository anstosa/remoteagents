...
const sourceSchema = z.object({
  ...
  launch: launchSchema,
  worktrees: z.array(z.object({ id: z.string().regex(/^[a-zA-Z0-9_-]{1,80}$/), label: z.string().max(120).optional(), path: z.string().min(1), launch: launchSchema.optional() }).strict()).min(1).max(100)
}).strict();
...
export async function validateConfig(input: unknown): Promise<ValidatedConfig> {
  ...
  for (const raw of parsed.worktrees) {
    ...
    const launch = raw.launch ?? parsed.launch; validateTemplate(launch); await access(launch.program, constants.X_OK);
    ...
    worktrees.push({ id: raw.id, label: raw.label ?? raw.id, path, identity, available: true, launch });
  }
