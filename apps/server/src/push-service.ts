...
export class PushService {
  ...
  async notify(message: PushMessage) { if (!this.enabled) return; const all = await this.read(); await Promise.all(Object.values(all).map(async subscription => { try { await webpush.sendNotification(subscription, JSON.stringify(message)); } catch (error) { const status = (error as { statusCode?: number }).statusCode; if (status === 404 || status === 410) { delete all[subscription.endpoint]; await this.write(all); } } })); }
