...
export async function buildApp(config: ValidatedConfig, deps: Dependencies = {}): Promise<FastifyInstance> {
  ...
  function controlled(request: FastifyRequest, mutation = false): Session { const s = session(request, mutation); if (!control.active(s.id)) throw inactiveClient(); return s; }
  ...
  app.get('/ws/logs/:id', { websocket: true }, async (socket, request) => { try { const s = controlled(request, false); const ticket = String(request.headers['sec-websocket-protocol'] ?? '').split(',').map(x => x.trim())[1]; const id = (request.params as { id: string }).id; if (!tickets.consume(ticket, s.id, 'logs', id)) throw new Error(); const target = await discovery.target(id); if (!target) throw new Error(); let last = ''; let lastResetAt = 0; let polling = false; const poll = async () => { if (polling) return; polling = true; try { if (!control.active(s.id)) return socket.close(1008); const value = await tmux.capture(target.socket, target.agent.paneId); if (value === undefined) return socket.close(1008); const frame = logFrame(last, value); if (!frame) return; last = value; if (frame.type === 'reset') { const now = Date.now(); if (lastResetAt && now - lastResetAt < 750) return; lastResetAt = now; } if (socket.readyState === socket.OPEN) socket.send(JSON.stringify({ v: 1, ...frame })); } finally { polling = false; } }; const timer = setInterval(() => { void poll(); }, config.pollIntervalMs); socket.on('close', () => clearInterval(timer)); await poll(); } catch { socket.close(1008); } });
