...
export type Worktree = { id: string; label: string; path: string; identity: string; hostPath?: string; available: boolean; pinned: boolean; command?: string; launch?: LaunchTemplate; projectUrl?: string; projectPort?: number; commands?: StackCommands; newTask?: string; push?: PromptAction };
...
export type Dashboard = { generation: number; agents: Agent[]; worktrees: Array<Pick<Worktree, 'id'|'label'|'path'|'available'|'pinned'|'projectUrl'> & { order: number; branch?: string; gitStatus?: GitStatusSummary; gitPrStatus?: GitComparisonSummary; pullRequest?: PullRequestSummary }> };
